...
class MySpiderd(Spider):
    ...
    def parse(self,response):
        ...
        dum=[]
        name=hxs.select('//div[@class="fclear line bmargin10"]/a/@href').extract()
        print name
            
        #url="www.flipkart.com"+d
        
        #file.close()
        #for r in csd:
        url=''.join(name)
        abc="http://flipkart.com"+url+"&start=0&sort_order=most-helpful"
        
        
        yield Request(abc,self.par_ur)

        
    def par_ur(self,response):
         hxs=HtmlXPathSelector(response)
         names=hxs.select('//a[@class="load-user-widget fk-underline"]')
         ...
         while(i<9): 
             ...
             p=hxs.select('//div[@class=" product-unit unit-3  browse-product  "]/@data-pid')
             item['pid']=p.extract()
                  
             #item['pid']=p.select('@data-pid').extract()
             #item['pid']=x
             ...
             h=hxs.select('//div[@class="unit"]/strong/text()')
             
             
             revid=hxs.select('//div[@class="review-list"]/a/@name')
             item['reviewid']=revid[i].extract()                            
             item['helpful']=h[n].extract()
             n=n+1
             item['totalreview']=h[n].extract()
             n=n+1
             items.append(item)
             i=i+1
             csk.writerow([item['pid'][0],item['rating'][0],item['userid'][0],item['date'][0],item['title'][0],item['rbody'][0],item['helpful'],item['totalreview'],item['reviewid']])
         ...
         for n in next_page:
             if n:
                yield Request(urlparse.urljoin("http://www.flipkart.com",n[1:]),callback=self.par_ur)  
